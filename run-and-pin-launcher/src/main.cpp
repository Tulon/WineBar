...
{
    // When an app is launched via the "Run & Pin" option, wine runs this launcher
    // first, which in turn runs the target executable. Before running the target
    // executable though, it extracts its icon and some metadata, and writes them
    // into the directory specified by the 1st command line argument. The rest of
    // the arguments define the target executable to run and its arguments.
    ...
    if (argc < 4)
    {
        // Q: Why do we need both <unix_or_windows_executable> and <program>?
        // A: Because when <unix_or_windows_executable> is not an .exe but something like
        //    .lnk or .msi, then <program> is going to be `start`. Yet, we want to extract
        //    the icon and the label specifically from <unix_or_windows_executable>.
        wprintf(
            L"Usage: %ls <unix_pin_dir> <unix_or_windows_executable> <program> [args...]\n",
            argv[0]);
        return 1;
    }
    ...
    try
    {
        ...
        return runProcess(argc - 3, argv + 3);
    }
    catch (WStringException const& e)
    {
        std::wcout << e.what() << std::endl;
